...
- VLAN table access is serialized per configured provider instance, but switch updates are not transactional and separate aliases or processes are not coordinated. Use `lifecycle { prevent_destroy = true }` for management-path VLANs.
